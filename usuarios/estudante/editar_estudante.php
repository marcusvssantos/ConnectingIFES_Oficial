...
try {
    ...
    $stmt->execute();
    $f_estudante = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($stmtEmail->rowCount() > 0) {
            ...
            if (isset($_FILES["fotoPerfil"]) && $_FILES["fotoPerfil"]["error"] === 0) {
                ...
                if (file_exists($f_estudante['fotoPerfil'])) {
                    unlink($f_estudante['fotoPerfil']);
                }
        ...
        <div class="container mt-5">
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="nome" name="nome" value="<?php echo $f_estudante['nome']; ?>">
                </div>
                <div class="form-group">
                    <label for="sobrenome">Sobrenome:</label>
                    <input type="text" class="form-control" id="sobrenome" name="sobrenome" value="<?php echo $f_estudante['sobrenome']; ?>">
                </div>
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" class="form-control" id="email" name="email" value="<?php echo $f_estudante['email']; ?>">
                    <?php if ($emailError) : ?>
